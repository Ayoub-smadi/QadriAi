...
export default function Auth() {
  ...
  const login = trpc.auth.login.useMutation({
    onSuccess: user => {
      ...
      setLocation(user?.role === "admin" ? "/control" : "/dashboard");
    },
  ...
  const submit = (event: FormEvent) => {
    ...
    login.mutate({ identifier, password, admin: isAdminLogin });
  };
  ...
  const copy = language === "ar" ? {
    ...
    username: "اسم المستخدم أو رقم الهاتف",
     usernamePlaceholder: "رقم هاتفك، أو Ayoub لحساب الأدمن",
    password: "كلمة المرور",
    passwordPlaceholder: "6 أحرف أو أكثر",
    confirm: "تأكيد كلمة المرور",
     submitLogin: isAdminLogin ? "دخول لوحة الأدمن" : "دخول آمن",
    ...
    username: "Username or phone number",
     usernamePlaceholder: "Your phone number, or Ayoub for admin",
    password: "Password",
    passwordPlaceholder: "6 characters or more",
    confirm: "Confirm password",
     submitLogin: isAdminLogin ? "Open admin control" : "Sign in securely",
