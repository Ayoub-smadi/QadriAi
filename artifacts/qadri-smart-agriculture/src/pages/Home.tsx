...
import { ArrowLeft, ArrowRight, Bot, CheckCircle2, ClipboardCheck, DraftingCompass, Globe2, Leaf, LineChart, Mail, Phone, ScanSearch, ShieldCheck, Sprout, TreePine, Waves } from "lucide-react";
      ...
      <section className="relative overflow-hidden border-b border-[#35530e]/10 bg-[#eff3e8]">
        ...
        <div className="container relative grid min-h-[610px] items-center gap-10 py-12 lg:grid-cols-[.92fr_1.08fr] lg:py-16">
          <div className="order-2 max-w-2xl lg:order-1">
            ...
            <div className="mt-8 flex flex-wrap gap-3"><Button asChild className="h-12 rounded-xl bg-[#35530e] px-5 text-base text-white shadow-[0_12px_24px_rgba(53,83,14,.22)] hover:bg-[#294108]"><Link href={isAuthenticated ? "/dashboard" : "/selector"}>{t.start}<Arrow className="ms-2 size-4" /></Link></Button><Button asChild variant="outline" className="h-12 rounded-xl border-[#35530e]/20 bg-white px-5 text-base text-[#35530e] hover:bg-[#f4f7f0]"><Link href="/engineer"><Bot className="me-2 size-4" />{language === "ar" ? "اسأل الذكاء الاصطناعي" : "Ask the AI"}</Link></Button>{!isAuthenticated && <Button onClick={() => setLocation("/auth")} variant="outline" className="h-12 rounded-xl border-[#35530e]/20 bg-white px-5 text-base text-[#35530e] hover:bg-[#f4f7f0]">{t.signIn}</Button>}</div>
          ...
          <div className="order-1 lg:order-2"><div className="relative mx-auto max-w-[720px]"><div className="absolute -inset-4 rounded-[2.25rem] bg-[#6c8b37]/15 blur-2xl" /><div className="relative aspect-[16/11] w-full overflow-hidden rounded-[2rem] border border-[#35530e]/10 bg-gradient-to-br from-[#f9fbf3] via-[#edf4df] to-[#dce9c3] shadow-[0_24px_60px_rgba(42,62,18,.18)]"><div className="absolute -end-10 -top-12 size-48 rounded-full bg-white/55 blur-2xl" /><div className="absolute -bottom-20 -start-12 size-56 rounded-full bg-[#b4ce65]/35 blur-3xl" /><span className="absolute start-[12%] top-[14%] grid size-14 place-items-center rounded-2xl border border-white/80 bg-white/80 p-1.5 shadow-sm motion-safe:animate-[qadri-float_5.8s_ease-in-out_infinite]" title={language === "ar" ? "ري ذكي موفّر للماء" : "AI smart irrigation"}><img src="/assets/ai-irrigation-sticker.png" alt={language === "ar" ? "ستيكر الري الذكي" : "Smart irrigation sticker"} className="size-full object-contain" /></span><span className="absolute end-[14%] top-[24%] grid size-11 place-items-center rounded-2xl border border-white/80 bg-white/80 p-1 shadow-sm motion-safe:animate-[qadri-float_5.4s_ease-in-out_.6s_infinite]" title={language === "ar" ? "مراقبة التربة بالذكاء الاصطناعي" : "AI soil monitoring"}><img src="/assets/ai-soil-sensor-sticker.png" alt={language === "ar" ? "ستيكر حساس التربة الذكي" : "AI soil sensor sticker"} className="size-full object-contain" /></span><span className="absolute end-[19%] bottom-[22%] grid size-14 place-items-center rounded-2xl border border-white/80 bg-white/80 p-1.5 shadow-sm motion-safe:animate-[qadri-float_6.6s_ease-in-out_1.1s_infinite]" title={language === "ar" ? "فحص المحصول بالذكاء الاصطناعي" : "AI crop scanning"}><img src="/assets/ai-crop-scan-sticker.png" alt={language === "ar" ? "ستيكر فحص المحصول بالذكاء الاصطناعي" : "AI crop scan sticker"} className="size-full object-contain" /></span><img src="/assets/ai-plant-icon.png" alt={language === "ar" ? "أيقونة نبات وذكاء اصطناعي" : "AI Plant icon"} className="absolute bottom-[7%] left-1/2 h-[64%] w-auto max-w-[70%] -translate-x-1/2 object-contain drop-shadow-[0_18px_12px_rgba(53,83,14,.2)] motion-safe:animate-[qadri-float_6s_ease-in-out_infinite]" /><div className="absolute bottom-3 start-3 max-w-[190px] rounded-xl border border-white/70 bg-white/90 p-2.5 shadow-lg backdrop-blur"><div className="flex items-center gap-1.5 text-[11px] font-bold text-[#35530e]"><Waves className="size-3.5" />{language === "ar" ? "مزرعة أذكى، بحجم أخف" : "A lighter, smarter farm visual"}</div></div></div></div></div>
        </div>
      </section>

      <section className="container py-12 sm:py-16"><div className="relative overflow-hidden rounded-[2rem] border border-[#35530e]/10 bg-[#f4f8ee] p-6 shadow-[0_16px_40px_rgba(48,67,22,.07)] sm:p-9" dir={language === "ar" ? "rtl" : "ltr"}><div className="pointer-events-none absolute -end-16 -top-20 size-48 rounded-full bg-[#dcebbd]/70 blur-3xl" /><div className="relative max-w-4xl"><p className="text-xs font-bold tracking-[.16em] text-[#759244]">{language === "ar" ? "رؤية زراعية من الأردن" : "AN AGRICULTURAL VISION FROM JORDAN"}</p><h2 className="mt-2 text-2xl font-bold tracking-tight text-[#293d12] sm:text-3xl">{language === "ar" ? "المهندس ثامر القادري" : "Engineer Thamer Al-Qadri"}</h2><p className="mt-5 min-h-[11rem] text-base leading-8 text-[#5f6d50] sm:min-h-[8rem]" aria-live="polite">{typedBio}<span className="ms-1 inline-block font-bold text-[#6d9335] animate-pulse">|</span></p><div className="mt-6 flex flex-wrap gap-3 text-sm font-semibold text-[#496327]"><a href="tel:0777772211" className="inline-flex items-center gap-2 rounded-full border border-[#bcd3a1] bg-white/80 px-4 py-2 transition hover:bg-white"><Phone className="size-4" />{language === "ar" ? "0777772211" : "0777772211"}</a><a href="mailto:tamerqadri@gmail.com" className="inline-flex items-center gap-2 rounded-full border border-[#bcd3a1] bg-white/80 px-4 py-2 transition hover:bg-white"><Mail className="size-4" />tamerqadri@gmail.com</a><a href="https://www.alqadrioffers.online" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#bcd3a1] bg-white/80 px-4 py-2 transition hover:bg-white"><Globe2 className="size-4" />{language === "ar" ? "الموقع الإلكتروني" : "Website"}</a></div></div></div></section>
