...
export default function FinancialDocuments() {
  ...
  return <AccessGate><AdminGate><PlatformShell title="الفواتير والسندات" eyebrow="إدارة المستندات المالية"><main className="container py-8" dir="rtl">
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1.35fr]">
      ...
      <section id="financial-document" className="rounded-2xl border-2 border-[#174d3b] bg-white p-5 shadow-sm sm:p-8"><div className="flex items-start justify-between gap-4 border-b-2 border-[#c8a75a] pb-5"><div className="flex items-center gap-3"><img src="/assets/qadri-logo.png" alt="شعار مؤسسة القادري الزراعية" className="size-16 object-contain" /><div><h2 className="text-2xl font-black text-[#174d3b]">مؤسسة القادري الزراعية</h2><p className="mt-1 text-sm text-[#60766b]">{doc.address || "الأردن"}</p></div></div><div className="text-end"><h1 className="text-2xl font-black text-[#174d3b]">{documentTypeLabel(type)}</h1><p className="mt-2 text-sm">رقم: <b>{doc.number}</b> · التاريخ: <b>{doc.date}</b></p></div></div>
