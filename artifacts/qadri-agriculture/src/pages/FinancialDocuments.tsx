...
function CatalogEditor({ onClose }: { onClose: () => void }) {
  ...
  const saveCatalog = () => { try { localStorage.setItem(CATALOG_STORAGE_KEY, JSON.stringify({ subtitle, logo, items })); setSavedMessage("تم حفظ الكتالوج بنجاح"); window.setTimeout(() => setSavedMessage(""), 2500); } catch (error) { console.warn("Could not save catalog", error); setSavedMessage("تعذر حفظ الكتالوج"); } };
