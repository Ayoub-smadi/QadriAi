...
function CatalogEditor({ onClose }: { onClose: () => void }) {
  ...
  const saveCatalog = () => { try { const entry = { id: catalogId(), title: subtitle || "كتالوج المنتجات الزراعية", savedAt: new Date().toLocaleString("ar-JO"), subtitle, logo, items }; const previous = JSON.parse(localStorage.getItem(CATALOG_HISTORY_KEY) || "[]") as typeof catalogHistory; const next = [entry, ...previous]; localStorage.setItem(CATALOG_STORAGE_KEY, JSON.stringify({ subtitle, logo, items })); localStorage.setItem(CATALOG_HISTORY_KEY, JSON.stringify(next)); setCatalogHistory(next); setCatalogView("history"); } catch (error) { console.warn("Could not save catalog", error); setSavedMessage("تعذر حفظ الكتالوج"); } };
