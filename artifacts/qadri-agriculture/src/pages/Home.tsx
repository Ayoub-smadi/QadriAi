...
import { ArrowLeft, ArrowRight, Bot, CheckCircle2, ClipboardCheck, DraftingCompass, Globe2, Leaf, LineChart, Mail, MapPin, Phone, ScanSearch, ShieldCheck, Sprout, TreePine } from "lucide-react";
...
export default function Home() {
  ...
  const dailyPlant = getDailyPlant();
  const plant = language === "ar" ? dailyPlant.ar : dailyPlant.en;
  const steps = [
  ...
  return <PlatformShell>
    <main>
      ...
      <section className="daily-plant-section container py-12 sm:py-16" dir={language === "ar" ? "rtl" : "ltr"}><div className="daily-plant-card"><div className="daily-plant-card__image-wrap"><img src={dailyPlant.image} alt={plant.name} className="daily-plant-card__image" /><span className="daily-plant-card__badge"><Leaf className="size-4" />{language === "ar" ? "نبتة اليوم" : "PLANT OF THE DAY"}</span></div><div className="daily-plant-card__body"><div className="flex flex-wrap items-center gap-3"><span className="daily-plant-card__eyebrow">{language === "ar" ? "اختيار القادري اليومي" : "TODAY'S AL-QADRI PICK"}</span><span className="daily-plant-card__scientific">{plant.scientific}</span></div><h2 className="mt-3 text-3xl font-black text-[#244b32] sm:text-4xl">{plant.name}</h2><p className="mt-3 text-base leading-7 text-[#607766]">{plant.summary}</p><div className="daily-plant-facts mt-6"><div><MapPin className="size-4" /><span><b>{language === "ar" ? "أين تعيش" : "Habitat"}</b>{plant.habitat}</span></div><div><Sprout className="size-4" /><span><b>{language === "ar" ? "موطنها الأصلي" : "Native range"}</b>{plant.origin}</span></div><div><Leaf className="size-4" /><span><b>{language === "ar" ? "طرق زراعتها" : "How to grow"}</b>{plant.growing}</span></div><div><ShieldCheck className="size-4" /><span><b>{language === "ar" ? "الحفاظ عليها" : "Care & protection"}</b>{plant.care}</span></div></div></div></div></section>
