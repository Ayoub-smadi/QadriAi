...
import { ArrowLeft, ArrowRight, Check, ChevronRight, Droplets, Leaf, Minus, Plus, Search, ShoppingBag, Sparkles, Store, Sun, Truck, X } from "lucide-react";
    ...
    <PlatformShell title={isArabic ? "طلب عرض سعر" : "Request a quote"} eyebrow={isArabic ? "اختيار نباتاتك صار أسهل" : "Choosing your plants, made easier"}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className={view === "catalog" ? "max-h-[92vh] max-w-3xl overflow-y-auto rounded-[1.6rem] border-[#cce0d6] bg-[#fbfdfb] p-5 sm:p-7" : view === "info" ? "max-h-[92vh] max-w-2xl overflow-y-auto rounded-[1.6rem] border-[#cce0d6] bg-[#fbfdfb] p-0" : "max-w-[420px] rounded-[1.6rem] border-[#cce0d6] bg-[#fbfdfb] p-5 sm:p-7"} dir={isArabic ? "rtl" : "ltr"}>
          {view === "catalog" ? (
            <>
              <DialogHeader className="border-b border-[#e0ebe5] pb-4"><DialogTitle className="flex items-center gap-2 text-xl font-extrabold text-[#17342d]"><ShoppingBag className="size-5 text-[#004132]" />{isArabic ? "خطوات طلب عرض السعر" : "Quote request steps"}</DialogTitle><DialogDescription className="text-sm text-[#668178]">{isArabic ? "اختر الطريقة والنباتات، وسنجهّز لك طلبًا واضحًا." : "Choose the method and plants, and we’ll prepare a clear request."}</DialogDescription></DialogHeader>
              <div className="mt-5 grid grid-cols-3 gap-2 text-center text-[11px] font-extrabold"><div className="rounded-lg bg-[#004132] px-2 py-2 text-white">01 · {isArabic ? "الطريقة" : "Method"}</div><div className={`rounded-lg px-2 py-2 ${selectedCount ? "bg-[#e1f0e9] text-[#17624d]" : "bg-[#eef5f1] text-[#7a9188]"}`}>02 · {isArabic ? "النباتات" : "Plants"}</div><div className="rounded-lg bg-[#eef5f1] px-2 py-2 text-[#7a9188]">03 · {isArabic ? "البيانات" : "Details"}</div></div>
              <fieldset className="mt-5"><legend className="text-sm font-extrabold text-[#17342d]">{isArabic ? "كيف تريد استلام طلبك؟" : "How would you like to receive your order?"}</legend><div className="mt-2 grid grid-cols-2 gap-3">{([["pickup", isArabic ? "استلام من المشتل" : "Nursery pickup", Store], ["delivery", isArabic ? "توصيل إلى موقعك" : "Delivery to your site", Truck] ] as const).map(([value, label, Icon]) => <button key={value} type="button" onClick={() => setRequestMethod(value)} aria-pressed={requestMethod === value} className={`flex min-h-16 items-center gap-2 rounded-xl border p-3 text-start text-xs font-extrabold transition ${requestMethod === value ? "border-[#57927c] bg-[#e7f3ed] text-[#17342d]" : "border-[#d8e5df] bg-white text-[#668178] hover:border-[#94bfad]"}`}><span className="grid size-8 place-items-center rounded-lg bg-[#f0f7f3]"><Icon className="size-4 text-[#17624d]" /></span>{label}{requestMethod === value && <Check className="ms-auto size-4 text-[#287459]" />}</button>)}</div></fieldset>
