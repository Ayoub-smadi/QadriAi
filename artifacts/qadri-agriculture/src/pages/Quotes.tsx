...
const quoteCategories: PlantCategory[] = ["fruit", "cactus", "trees", "shrubs", "flowers", "medicinal"];
  ...
  return (
    <PlatformShell title={isArabic ? "نباتاتك تبدأ من هنا" : "Your plants start here"} eyebrow={isArabic ? "اختيار واضح، عرض سعر أدق" : "A clear selection for a precise quote"}>
        ...
        <div className="container py-8 sm:py-12">
          ...
          <section className="mt-10">
            <div className="flex flex-col gap-4 border-b border-[#dfe9df] pb-6 sm:flex-row sm:items-end sm:justify-between">
              ...
              <Button onClick={openCatalog} className="h-12 shrink-0 rounded-xl bg-[#0a4b39] px-6 font-black text-white shadow-[0_10px_24px_rgba(10,75,57,.22)] hover:bg-[#063b2d]"><ShoppingBag className="me-2 size-5" />{isArabic ? "طلب عرض سعر" : "Request a quote"}<ArrowLeft className="ms-2 size-4 rtl:rotate-180" /></Button>
            </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className={view === "catalog" ? "max-h-[calc(100dvh-1rem)] max-w-3xl overflow-y-auto rounded-[1.6rem] border-[#cce0d6] bg-[#fbfdfb] p-5 sm:p-7" : view === "info" ? "max-h-[calc(100dvh-1rem)] max-w-2xl overflow-y-auto rounded-[1.6rem] border-[#cce0d6] bg-[#fbfdfb] p-0" : "max-w-[420px] rounded-[1.6rem] border-[#cce0d6] bg-[#fbfdfb] p-5 sm:p-7"}>
          {view === "catalog" ? <>
