    ...
    <main className="container py-6 sm:py-8" dir={ar ? "rtl" : "ltr"}>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_370px]">
        <section className="order-2 rounded-[1.6rem] border border-[#dce6d7] bg-white p-5 shadow-[0_12px_35px_rgba(35,65,35,.06)] xl:order-1 sm:p-7">
          <div className="flex items-end justify-between gap-3 border-b border-[#edf2e9] pb-5"><div><p className="text-xs font-black tracking-[.12em] text-[#78924a]">{ar ? "مساحة العمل" : "WORKSPACE"}</p><h2 className="mt-2 text-2xl font-black text-[#294a2e]">{ar ? "ابنِ تصورك على أرضك" : "Build the concept on your land"}</h2></div><span className="hidden rounded-full bg-[#f1f7e9] px-3 py-1.5 text-xs font-bold text-[#6b8543] sm:inline-flex">{ar ? "محاصيل · ري · ممرات" : "Crops · irrigation · paths"}</span></div>
          <div className="mt-5 rounded-[1.4rem] border border-[#d9e7cd] bg-gradient-to-br from-[#f5f9ef] via-white to-[#edf6ec] p-5 sm:p-6">
            <div className="flex items-start gap-3"><div className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#073f35] text-xl text-white shadow-lg">✦</div><div><h3 className="text-base font-black text-[#294a2e]">{ar ? "ما الذي سيظهر في الصورة؟" : "What will appear in the image?"}</h3><p className="mt-1 text-xs leading-6 text-[#718062]">{ar ? "تصور واقعي يحافظ على تضاريس الموقع ويضيف العناصر التي تصفها أنت." : "A realistic concept that preserves the site and adds what you describe."}</p></div></div>
            <div className="mt-5 grid gap-2 sm:grid-cols-3"><FeaturePill icon="🌱" ar="صفوف محاصيل" en="Crop rows" /><FeaturePill icon="💧" ar="شبكة ري" en="Irrigation" /><FeaturePill icon="↔" ar="ممرات خدمة" en="Service paths" /></div>
          </div>
...
function FeaturePill({ icon, ar, en }: { icon: string; ar: string; en: string }) {
  return <div className="rounded-xl border border-[#e0ead8] bg-white/75 p-3 text-center"><span className="block text-xl">{icon}</span><span className="mt-1 block text-xs font-bold text-[#617354]">{ar}</span><span className="sr-only">{en}</span></div>;
}
