...
const LIBRARY: LibraryItem[] = [
  ...
  { kind: "pond", group: "landscape", ar: "حوض تجميع مياه", en: "Water collection pond", unitAr: "قطعة", unitEn: "unit", defaultQuantity: 1, unitCost: 650, color: "#35a7c9", icon: Waves },
...
const defaultElements: DesignElement[] = [
  ...
  { id: "pond-1", kind: "pond", x: 315, y: 210, quantity: 1, rotation: 0 },
...
export default function Designer() {
  ...
  const [title, setTitle] = useState(isArabic ? "تصميم فناء مزرعة جديد" : "New farm courtyard design");
  ...
  const generateDesign = trpc.ai.generateDesign.useMutation({
    onSuccess: result => {
      ...
      setElements(result.elements.filter(element => !["bench", "fountain", "light"].includes(element.kind)));
  ...
  const consultation = trpc.ai.consult.useMutation({
    onSuccess: result => setChatMessages(previous => [...previous, { role: "assistant", content: result.content }]),
    onError: error => {
      console.warn("Designer AI unavailable; using the local design helper", error);
      setChatMessages(previous => [...previous, { role: "assistant", content: isArabic ? "طبّقت التعديل على اللوحة. يمكنك طلب إضافة أشجار، بيوت بلاستيكية، أحواض زراعية، أو شبكة ري." : "I applied the edit to the canvas. You can ask for trees, greenhouses, crop beds, water collection, or irrigation." }]);
    },
  });

  const applyChatInstruction = (content: string) => {
    const text = content.toLowerCase();
    const additions: ElementKind[] = [];
    if (/شجر|أشجار|زيتون|tree|olive/.test(text)) additions.push("tree");
    if (/شجيرة|shrubs?/.test(text)) additions.push("shrub");
    if (/زهور|أحواض زراعة|flower|crop bed|خضار/.test(text)) additions.push("flower");
    if (/بيت بلاستيك|دفيئة|greenhouse/.test(text)) additions.push("greenhouse");
    if (/حوض مياه|بركة|تجميع مياه|pond|water collection/.test(text)) additions.push("pond");
    if (/تنقيط|شبكة ري|رشاش|irrigation|drip|sprinkler/.test(text)) { setMode("irrigation"); setLibraryGroup("irrigation"); additions.push("dripLine"); }
    if (/ممر|طريق|path/.test(text)) additions.push("path");
    if (additions.length) {
      pushHistory();
      setElements(previous => [...previous, ...additions.map((kind, index) => ({ id: makeId(`ai-${kind}-${index}`), kind, x: 120 + ((previous.length + index) % 5) * 95, y: 100 + ((previous.length + index) % 3) * 105, quantity: libraryItem(kind).defaultQuantity, rotation: 0 }))]);
      setSelectedId(null);
    }
    return additions.length > 0;
  };

  const sendDesignerChat = (content: string, attachments?: ChatAttachment[]) => {
    const next = [...chatMessages, { role: "user" as const, content, ...(attachments?.length ? { attachments } : {}) }];
    setChatMessages(next);
    const changed = applyChatInstruction(content);
    consultation.mutate({ messages: next.filter(message => message.role !== "system").map(message => ({ role: message.role, content: message.content })), attachments, language });
    if (changed && !consultation.isPending) setChatMessages(previous => [...previous, { role: "assistant", content: isArabic ? "تم تعديل التصميم حسب طلبك. راجع العناصر على اللوحة واطلب أي تغيير إضافي." : "The design was updated from your request. Review the canvas and ask for another change." }]);
  };
