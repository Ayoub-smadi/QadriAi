  ...
  return <main className="quote-editor container py-8 [&_input]:text-center [&_textarea]:text-center">
    ...
    <div className="grid gap-6 xl:grid-cols-[.95fr_1.05fr]">
      <section className="space-y-5">
        <article className="rounded-[1.5rem] border border-[#35530e]/10 bg-[#f7fbf3] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold tracking-[.14em] text-[#78924a]">{isArabic ? "التحكم بالعرض" : "QUOTE CONTROLS"}</p><h2 className="mt-1 text-xl font-bold text-[#314617]">{isArabic ? "أضف بندًا أو رسوم شحن" : "Add an item or shipping"}</h2></div><Button onClick={addItem} className="h-10 rounded-xl bg-[#35530e] font-extrabold text-white hover:bg-[#294108]"><Plus className="me-1 size-4" />{isArabic ? "إضافة صف" : "Add row"}</Button></div><div className="mt-4 max-w-xs"><Label>{isArabic ? "رسوم الشحن (د.أ)" : "Shipping fee (JOD)"}</Label><Input type="number" min="0" step="0.01" value={record.shippingFee} onChange={event => update("shippingFee", Math.max(0, Number(event.target.value) || 0))} className="mt-1.5 h-10 rounded-xl" /><p className="mt-1 text-xs text-[#718062]">{isArabic ? "تظهر في المجموع عند اختيار طريقة التوصيل." : "Included in the total when delivery is selected."}</p></div></article>
        <article className="rounded-[1.5rem] border border-[#35530e]/10 bg-white p-5"><Label>{isArabic ? "صيغة طريقة الاستلام في العرض" : "Fulfillment wording on the quote"}</Label><Input value={record.fulfillmentLabel || (record.fulfillment === "delivery" ? "توصيل" : "استلام من المشتل")} onChange={event => update("fulfillmentLabel", event.target.value)} placeholder={isArabic ? "مثال: استلام من موقعنا أو توصيل إلى العنوان" : "e.g. Pickup from our nursery or delivery to address"} className="mt-1.5 h-10 rounded-xl" /><p className="mt-1 text-xs text-[#718062]">{isArabic ? "اكتب الصيغة التي تريد ظهورها بدل كلمة الطريقة." : "Write the exact wording you want to show instead of the default label."}</p></article>
        <article className="rounded-[1.5rem] border border-[#35530e]/10 bg-white p-5"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-lg bg-[#edf4e5] text-[#52731f]"><ReceiptText className="size-5" /></span><div><p className="text-xs font-bold tracking-[.14em] text-[#78924a]">{isArabic ? "هوية العرض" : "QUOTE IDENTITY"}</p><h2 className="mt-1 text-xl font-bold text-[#314617]">{isArabic ? "رقم وتاريخ العرض" : "Quote number & date"}</h2></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><div><Label>{isArabic ? "رقم العرض" : "Quote number"}</Label><Input dir="ltr" value={record.quoteNumber} onChange={event => update("quoteNumber", event.target.value)} className="mt-1.5 h-10 rounded-xl text-center" /></div><div><Label>{isArabic ? "تاريخ العرض" : "Quote date"}</Label><Input type="date" value={(record.issueDate || record.updatedAt).slice(0, 10)} onChange={event => update("issueDate", event.target.value)} className="mt-1.5 h-10 rounded-xl text-center" /></div></div></article>
