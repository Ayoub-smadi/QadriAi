...
function Editor({ record, setRecord, onSave, onDownload, downloading, sheetRef, onClose, language }: { record: QuoteRecord; setRecord: (record: QuoteRecord) => void; onSave: () => void; onDownload: () => void; downloading: boolean; sheetRef: RefObject<HTMLDivElement | null>; onClose: () => void; language: "ar" | "en" }) {
  ...
  return <main className="quote-editor container py-8 [&_input]:text-center [&_textarea]:text-center">
    ...
    <style>{`.quote-pages .quote-page { box-sizing: border-box; overflow: hidden; } .quote-density-2 { font-size: 9px; } .quote-density-2 [data-quote-header="true"] { padding-bottom: 10px; } .quote-density-2 table { margin-top: 10px; font-size: 8px; } .quote-density-2 th, .quote-density-2 td { padding: 3px 4px !important; line-height: 1.25 !important; } .quote-density-2 .quote-end-block > div { margin-top: 10px; } .quote-density-3 { font-size: 8px; } .quote-density-3 [data-quote-header="true"] { padding-bottom: 6px; } .quote-density-3 [data-quote-header="true"] img { height: 58px; margin-bottom: 4px; } .quote-density-3 table { margin-top: 6px; font-size: 7px; } .quote-density-3 th, .quote-density-3 td { padding: 2px 3px !important; line-height: 1.15 !important; } .quote-density-3 .quote-end-block > div { margin-top: 6px; } .quote-density-4 { font-size: 7px; } .quote-density-4 [data-quote-header="true"] { padding-bottom: 4px; } .quote-density-4 [data-quote-header="true"] img { height: 44px; margin-bottom: 2px; } .quote-density-4 table { margin-top: 4px; font-size: 6px; } .quote-density-4 th, .quote-density-4 td { padding: 1px 2px !important; line-height: 1.05 !important; } .quote-density-4 .quote-end-block > div { margin-top: 4px; }`}</style>
<div className="rounded-[1.5rem] border border-[#35530e]/10 bg-[#e9eee2] p-3 shadow-[0_14px_35px_rgba(48,67,22,.08)]"><div className="mb-3 px-2 text-sm font-bold text-[#405525]">{isArabic ? "عدّل البيانات مباشرة داخل صفحة العرض" : "Edit the quote directly on the page"}</div><div ref={sheetRef} className="quote-pages overflow-auto rounded-xl bg-[#dfe7d8] p-4"><div className="grid justify-center gap-5"><QuoteDocument className={`quote-page quote-density-${Math.min(4, Math.max(1, Math.ceil(record.items.length / 5)))} min-h-[297mm] h-[297mm] w-[210mm] bg-white p-[12mm] shadow-[0_8px_24px_rgba(30,60,35,.12)]`} record={record} showFooter editable onChange={patch => setRecord({ ...record, ...patch })} onItemChange={(itemId, patch) => update("items", record.items.map(item => item.id === itemId ? { ...item, ...patch } : item))} onImageChange={(itemId, dataUrl) => update("items", record.items.map(item => item.id === itemId ? { ...item, imagePath: dataUrl } : item))} onAddItem={addItem} onRemoveItem={removeItem} onLogoChange={dataUrl => update("logoPath", dataUrl)} onStampChange={dataUrl => update("stampPath", dataUrl)} /></div></div></div>
