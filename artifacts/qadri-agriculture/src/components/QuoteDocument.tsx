...
export const QuoteDocument = forwardRef<HTMLDivElement, QuoteDocumentProps>(function QuoteDocument({ record, className = "" }, ref) {
  ...
  return (
    <div ref={ref} dir={isArabic ? "rtl" : "ltr"} className={`quote-document w-full bg-white p-7 text-[#22351b] ${className}`}>
      ...
      <div className="mt-7 border-t border-[#d9e3d1] pt-5 text-center leading-6"><p className="text-sm font-bold text-[#22351b]">{record.closingText || "واقبلوا فائق الاحترام والتقدير،،،"}</p>{record.footerText && <p className="mt-2 text-xs text-[#68775a]">{record.footerText}</p>}{record.notes && <p className="mt-3 text-xs text-[#68775a]">{record.notes}</p>}</div>
