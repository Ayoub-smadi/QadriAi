...
export const QuoteDocument = forwardRef<HTMLDivElement, QuoteDocumentProps>(function QuoteDocument({ record, className = "" }, ref) {
  ...
  return (
    <div ref={ref} dir={isArabic ? "rtl" : "ltr"} className={`quote-document w-full bg-white p-7 text-[#22351b] ${className}`}>
      ...
      {(record.notes || record.footerText || record.closingText) && <div className="mt-7 border-t border-[#d9e3d1] pt-4 text-xs leading-6 text-[#68775a]">{record.notes && <p><strong>{isArabic ? "ملاحظات" : "Notes"}:</strong> {record.notes}</p>}{record.footerText && <p className="mt-1">{record.footerText}</p>}{record.closingText && <p className="mt-5 text-sm font-bold text-[#22351b]">{record.closingText}</p>}</div>}
      <div dir="ltr" className="mt-7 flex items-end justify-between gap-8 border-t border-[#d9e3d1] pt-5 text-xs text-black"><img src={record.stampPath || "/assets/qadri-stamp.png"} alt="Stamp" className="h-20 w-auto max-w-[180px] object-contain object-left" /><div dir="rtl" className="space-y-1 text-right"><p className="font-bold">مؤسسة القادري الزراعية</p><p className="flex items-center justify-end gap-2"><Phone className="size-3.5 text-black" />00962777772211</p><p className="flex items-center justify-end gap-2"><Mail className="size-3.5 text-black" />tamerqadri@gmail.com</p><p className="flex items-center justify-end gap-2"><Globe className="size-3.5 text-black" />https://www.alqadrioffers.online</p></div></div>
    </div>
  );
});
