...
export const QuoteDocument = forwardRef<HTMLDivElement, QuoteDocumentProps>(function QuoteDocument({ record, className = "", editable = false, onChange, onItemChange }, ref) {
  ...
  const edit = (value: string, save: (value: string) => void, multiline = false) => editable ? (multiline ? <textarea defaultValue={value} onBlur={event => save(event.target.value)} className="min-h-16 w-full resize-y rounded border border-[#c9d9bf] bg-white/80 p-1 text-inherit outline-none" /> : <input defaultValue={value} onBlur={event => save(event.target.value)} className="w-full min-w-0 rounded border border-[#c9d9bf] bg-white/80 p-1 text-inherit outline-none" />) : value;
      ...
      <table className="mt-6 w-full min-w-[980px] table-fixed border-collapse text-xs [word-break:normal] [overflow-wrap:break-word]">
        ...
        <tbody>{record.items.map((item, index) => <tr key={item.id} className="align-top even:bg-[#f8faf5]">{columns.filter(visible).map(column => {
          const content: Record<QuoteColumnKey, ReactNode> = {
            ...
            quantity: editable ? <input type="number" min="1" defaultValue={item.quantity} onBlur={event => onItemChange?.(item.id, { quantity: Math.max(1, Number(event.target.value) || 1) })} className="w-16 rounded border border-[#c9d9bf] p-1" /> : item.quantity,
            price: editable ? <input type="number" min="0" step="0.01" defaultValue={item.price} onBlur={event => onItemChange?.(item.id, { price: Math.max(0, Number(event.target.value) || 0) })} className="w-24 rounded border border-[#c9d9bf] p-1" /> : money(item.price),
