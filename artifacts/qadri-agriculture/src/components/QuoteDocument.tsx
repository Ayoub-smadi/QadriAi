  ...
  return (
    <div ref={ref} dir={isArabic ? "rtl" : "ltr"} className={`quote-document w-full bg-white p-7 text-[#22351b] ${className}`}>
      <div data-quote-header="true" className="flex items-start justify-between gap-5 border-b-2 border-[#35530e] pb-5">
        ...
        <div className="text-left text-base"><p className="text-lg font-extrabold text-[#35530e]">{isArabic ? "رقم العرض" : "Quote no."}</p><p dir="ltr" className="mt-1 text-xl font-black tracking-wide text-[#22351b]">{record.quoteNumber}</p><p className="mt-2 text-sm text-[#6c7b62]">{new Date(record.updatedAt).toLocaleDateString(isArabic ? "ar-JO" : "en-US")}</p></div>
      </div>
