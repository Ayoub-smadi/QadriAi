  ...
  return (
    <div ref={ref} dir={isArabic ? "rtl" : "ltr"} className={`quote-document w-full bg-white p-7 text-[#22351b] ${className}`}>
      <div data-quote-header="true" className="flex items-start justify-between gap-5 border-b-2 border-[#35530e] pb-5">
        <div className="flex-1"><img src={record.logoPath || "/assets/qadri-logo.png"} alt="Al-Qadri" className="mb-4 h-24 w-auto object-contain" /><div dir="ltr" className="flex items-center justify-between gap-5 text-lg font-extrabold text-[#35530e]"><span>Al-Qadri Agricultural Establishment</span><span dir="rtl">مؤسسة القادري الزراعية</span></div><h1 className="mt-2 text-2xl font-extrabold">{record.title || (isArabic ? "عرض سعر نباتات" : "Plant quotation")}</h1></div>
        <div className="text-left text-lg"><p className="text-2xl font-extrabold text-[#35530e]">{isArabic ? "رقم العرض" : "Quote no."}</p><p dir="ltr" className="mt-2 text-3xl font-black tracking-wide text-[#22351b]">{record.quoteNumber}</p><p className="mt-2 text-base text-[#6c7b62]">{new Date(record.updatedAt).toLocaleDateString(isArabic ? "ar-JO" : "en-US")}</p></div>
      </div>
      ...
      <div dir="ltr" className="mt-7 flex items-end justify-between gap-8 border-t border-[#d9e3d1] pt-5 text-xs text-black"><img src={record.stampPath || "/assets/qadri-stamp.png"} alt="Stamp" className="h-20 w-auto max-w-[180px] object-contain object-left" /><div dir="rtl" className="space-y-1 text-right"><p className="font-bold">مؤسسة القادري الزراعية</p><p className="flex items-center justify-end gap-2"><Phone className="size-3.5 text-black" />00962777772211</p><p className="flex items-center justify-end gap-2"><Mail className="size-3.5 text-black" />tamerqadri@gmail.com</p><p className="flex items-center justify-end gap-2"><Globe className="size-3.5 text-black" />https://www.alqadrioffers.online</p></div></div></div>
    </div>
  );
});
