...
**How to apply:** Keep auth routes mounted under the API artifact's `/api` router, preserve credential cookies for the browser session, and verify the proxied `/api/trpc/auth.me`, `auth.login`, and `auth.register` endpoints after workflow changes.